...
Deployment readiness owner action intake artifacts can be exported with `node scripts/export-deployment-readiness-owner-action-intake-artifact.js` for later owner action intake review only; the artifact accepts no secrets or real owner values; not deployment approval, not funded transaction approval, not governance transaction approval, not BscScan verification approval, not release approval, not visibility approval, and not readiness.
Deployment readiness owner action intake artifact snapshots can be run with `node scripts/test-deployment-readiness-owner-action-intake-artifact-snapshot.js` to lock compact and pretty JSON outputs, blocked/pending statuses, safeTo flags, no-secret and no-real-owner-values state, owner review requirement, later deploy instruction requirement, non-approval boundaries, and summary counts.
Deployment readiness owner action intake final gate can be run with `node scripts/check-deployment-readiness-owner-action-intake-final-gate.js` as the final placeholder-only gate before any future real owner value intake; it accepts no secrets or real owner values, does not approve deployment, funded transactions, governance transactions, BscScan verification, releases, visibility changes, or readiness claims.
Deployment readiness owner action intake final gate snapshots can be run with `node scripts/test-deployment-readiness-owner-action-intake-final-gate-snapshot.js` to lock the text and JSON outputs, blocked upstream statuses, safeTo flags, owner review requirement, later explicit deploy instruction requirement, no-secret and no-real-owner-values state, and non-approval boundaries.
[Deployment readiness owner action intake runbook](docs/deployment-readiness-owner-action-intake-runbook.md) is owner-facing guidance for later public owner decision intake only; it accepts no secrets and no real owner values in this PR, and it is not deploy approval, not funded transaction approval, not governance transaction approval, not BscScan verification approval, not release approval, not visibility approval, and not readiness.
[Testnet deployment candidate manifest](docs/testnet-deployment-candidate-manifest.md) fingerprints the future BNB Smart Chain testnet deployment candidate without network calls, raw bytecode, real owner values, secrets, deploy approval, funded transaction approval, governance transaction approval, BscScan verification approval, release approval, visibility approval, or readiness approval.
